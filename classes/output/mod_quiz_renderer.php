...
class mod_quiz_renderer extends \mod_quiz\output\renderer
{
    public function view_information($quiz, $cm, $context, $messages, bool $quizhasquestions = false)
    {
                ...
                $is_student  = !has_capability('moodle/course:manageactivities', $context);
		...
		// Mostra il bottone solo se SEB è attivo nelle configurazioni del quiz
                if ($attempts && $seb_enabled && $is_student) {
		    $logouturl = new moodle_url('/login/logout.php',['sesskey' => sesskey()]);
                    $output .= html_writer::div(
		    html_writer::tag('p', 'Se hai terminato l\'esame o non hai superato la prova, clicca il pulsante per effetuare il logout.').
		    html_writer::tag('a', 'LOGOUT', [
			'href' => $logouturl->out(false),
