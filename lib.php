...
function theme_academi_page_init(moodle_page $page) {
    ...
    // Debug in console
    $page->requires->js_init_code("console.log('🚀 Academi Breadcrumb: Inizio ricostruzione...');");

    // 1. Filtri di contesto
    if ($page->context->contextlevel != CONTEXT_COURSE || $COURSE->id == SITEID) {
        return;
    }

    // 2. Solo per i non iscritti (Guest o utenti che guardano il catalogo)
    if (is_enrolled($page->context)) {
        $page->requires->js_init_code("console.log('ℹ️ Utente iscritto: uso breadcrumb standard.');");
        return;
    }

    try {
        // Recuperiamo la gerarchia delle categorie
        $category = core_course_category::get($COURSE->category);
        $parents = $category->get_parents();
       
        // --- TRUCCO PER L'ORDINE ---
        // Diciamo a Moodle di ignorare i nodi automatici per un momento
        $page->navbar->ignore_active(true);
       
        // Aggiungiamo manualmente il link alla Home
        $page->navbar->add(get_string('home'), new moodle_url('/'));

        // Aggiungiamo tutte le categorie antenate
        foreach ($parents as $parentid) {
            $parent = core_course_category::get($parentid);
            if (!$parent->visible) continue;
            $page->navbar->add(
                format_string($parent->name),
                new moodle_url('/course/index.php', ['categoryid' => $parent->id]),
                navigation_node::TYPE_CATEGORY
            );
        }

        // Aggiungiamo la categoria del corso
        if ($category->visible) {
            $page->navbar->add(
        ...
        // Infine aggiungiamo il Corso corrente (come nodo finale non cliccabile)
        $page->navbar->add(format_string($COURSE->fullname));

        $page->requires->js_init_code("console.log('✅ Breadcrumb ricostruiti: Home > Categorie > Corso');");

    } catch (Exception $e) {
        $page->requires->js_init_code("console.error('❌ Errore: " . addslashes($e->getMessage()) . "');");
    }
}
